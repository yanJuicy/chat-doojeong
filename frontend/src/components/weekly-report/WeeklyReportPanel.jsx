...
export default function WeeklyReportPanel({ weeklyReport }) {
  const {
    department,
    departments,
    setDepartment,
  ...
  return (
    <section className="weekly-report-panel" aria-labelledby="weekly-report-title">
      <div className="panel-intro">
        <p className="eyebrow">WEEKLY REPORT</p>
        <h3 id="weekly-report-title">주간보고서 모드</h3>
          ...
          <section className="weekly-report-list">
            ...
            <ul>
              {currentWeekEntries.map((entry) => (
                <EntryRow key={entry.id} entry={entry} onEdit={editEntry} onDelete={removeEntry} />
              ))}
            </ul>
          </section>
