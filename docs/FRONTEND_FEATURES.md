...
- 대화 목록/메시지 내용 자체는 여전히 브라우저에만 저장된다(다른 PC·브라우저·사용자와 화면 공유는 안 됨).
  다만 각 대화의 `id`(client가 만든 UUID)는 이제 질문을 보낼 때 `session_id`로 백엔드에 같이 전달된다
  (`useChat.submitQuestion` → `api.streamQuestion`) — 백엔드가 이 값으로 서버 쪽 `chat_turns` 테이블에
  대화 이력을 쌓고, "그럼 무게는?" 같은 후속 질문을 이전 turn을 참고해 재작성한다. 즉 **대화 내용을
  다른 기기에서 이어보는 기능은 아직 없지만, 같은 브라우저 안에서는 후속 질문이 문맥을 이해하게 됐다.**
  자세한 내용은 `docs/hanju/api/multi_turn_chat_api.md` 참고.
